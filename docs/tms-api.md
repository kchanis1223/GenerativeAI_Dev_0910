...
- 실제 API 전환 시 인증 방식, 배열/객체, 숫자 타입, 처리 중 응답, 에러 코드 및 결과 준비 시간을 실제 응답으로 확인하고 어댑터를 연결해야 합니다. 이 문서의 24개 CRUD·배차 화면 API는 목업이며 센터 목록에는 별도 프록시가 있습니다. Python Agent는 별도로 실제 TMAP 지오코딩·TMS 배차 접수·결과 조회에 연결되어 있습니다. [Agent 실행 안내](agent-integration.md)의 서버 설정과 실연동 범위를 따릅니다.
