...
목업 계산은 배송 준비 화면에서 선택한 센터에서 출발해 가까운 배송지 순으로 배분합니다. 차량 유형, 권역, 투입 여부, 적재 한도, 교차금지선을 검사하고 선택한 배차·균등화 기준과 숙련도로 차량을 선택합니다. `weight * 1000`으로 단위를 맞추며 부피 0은 제한 미설정, 빈 권역은 권역 제한 없음으로 간주합니다. 숙련도는 배분 점수에만 반영합니다. 차량 별도 도착 좌표가 있으면 이를 우선하고, 없으면 복귀 설정을 따릅니다.
...
배송 준비 화면은 센터→주문→차량→조건→요청→계산→결과→차량 상세의 8단계로 진행합니다. 선택한 ID 목록만 `allocationType: "2"`로 요청하며, 센터는 공식 요청 필드와 분리된 `MockContext.centerId`로 전달합니다. 실행 로그의 `mockContext`에서 선택 센터를 확인할 수 있습니다. API 탐색 화면처럼 별도 센터 실행 정보가 없는 호출은 첫 등록 센터를 기본값으로 사용합니다. 등록 센터의 배열 순서는 변경하지 않습니다.
