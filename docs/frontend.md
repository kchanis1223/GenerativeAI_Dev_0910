...
기존 `executeTms(state, '/orderList')`와 같은 필드명으로 CSV를 읽습니다. Python 채팅은 `agent/badaro/runtime/tools.py`의 `get_delivery_orders`를 통해 별도 CSV 조회를 수행합니다. 주소 20곳과 센터의 지오코딩 원본 응답 및 대조 결과, 전체 컬럼은 [데이터 안내서](../data/README.md)를 참고하세요.
...
- 기존 프론트 목업 선택값은 채팅에 적용되지 않습니다. 지도는 Python 결과의 확정 좌표를 점선으로 표시하며 목업과 전환할 수 있습니다. Python v2가 제공하지 않는 기사 역할·재배차도 미구현입니다.
