...
채팅 배차 결과는 확정 좌표를 방문 순서대로 연결한 점선으로 지도에 표시합니다. 차량별 색상·순서 번호를 표시하며 실제 도로 경로는 조회하지 않습니다. 지도에서 ‘에이전트 결과’와 ‘목업 데이터’를 전환할 수 있습니다. 아래 프론트 목업의 선택값은 채팅 요청에 적용되지 않습니다. 새 요청은 ‘새 대화’로 시작하며, 응답이 유실되면 실행 여부를 확인하기 전 재전송하지 마세요. 재배차·기사 권한 검증은 현재 서버 범위에 포함되지 않습니다.
