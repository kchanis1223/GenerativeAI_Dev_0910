...
압축을 푼 새 Python 3.14 가상환경에서 README 설치 명령을 확인했다. wheel 검사가 사용하는 setuptools가 개발 환경에 빠져 1개가 실패했고, `dev` 의존성에 빌드 설정과 같은 범위를 추가한 뒤 191개가 모두 통과했다. 새 프론트 설치·빌드와 압축 해제 소스의 Mock HTTP 시연도 통과했다.
