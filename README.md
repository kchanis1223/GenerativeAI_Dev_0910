...
채팅의 실행 모드는 Python 서버 응답을 표시합니다. M01~M08과 지도까지 시연하려면 `npm run agent:demo`로 고정 시연 서버를 실행하세요. [시연 입력·답변과 검증 명령](docs/demo-scenarios.md)을 참고하세요. 시연 모드에서는 현재 요청에서 확인한 주문·차량·좌표를 아래 선택 목록·결과·지도에 반영합니다. 새 요청은 ‘새 대화’로 시작합니다. 재배차·기사 권한 검증은 현재 서버 범위에 포함되지 않습니다.
