...
사용자 요청은 프론트에서 API 응답에 따른 동작 로직을 시연하는 것입니다. 첨부 양식의 미작성 항목은 구현 요구 사항으로 간주하지 않았습니다. 센터 조회에 더해 차량·배송지·권역·교차금지선과 배차 요청을 포함합니다. [24개 API와 확장 목업 설계](tms-api.md)를 함께 참고하세요.
...
메인은 물 표면 사진·일렁이는 진입 링크인 바다로 로고와 주변부의 작은 물고기 점프 효과로 구성합니다. 내부 화면은 제공된 신청서 화면 레퍼런스의 상단 가로 내비게이션, 큰 페이지 제목, 캡슐 모양 구간 표시, 좌측 입력 카드와 우측 테두리 요약 카드 구조를 따릅니다. 레퍼런스의 신청자명·휴대폰·결제 요소 대신 기존 센터 조회 기능을 배치합니다.
...
## 배송 준비를 중심으로 한 재배치 (2026-09-11)

메인 로고의 진입 대상 `/workspace`를 8단계 배송 준비 화면으로 변경했습니다. 기존 센터 조회는 `/workspace/centers`, 주문·차량 정보와 API 탐색·실행 기록은 보조 메뉴로 유지합니다.

```mermaid
flowchart TD
  A[1. 센터 선택] --> B[2. 오늘 배송할 주문 등록/선택]
  B --> C[3. 운행 가능한 차량 선택]
  C --> D[4. 배차 조건 설정]
  D --> E[5. 배차 요청 전 최종 확인]
  E --> F[6. 계산 중]
  F --> G[7. 배차 결과·미배차 사유]
  G --> H[8. 차량별 순서·예상시간·경로]
  F -->|요청 실패·조회 한도 초과| E
```

진행 상태와 선택 정보는 `dispatch-workflow.ts`에 보관합니다. 완료한 준비 단계는 재방문할 수 있으며 선택 변경 시 이전 결과를 무효화합니다. 계산은 요청 시점 스냅샷으로 실행하고, 대기 중 데이터 변경이 발생하면 응답을 화면에 적용하지 않습니다. 필수 선택·운행 여부·시간을 검사하며, 조회는 최대 8회로 제한합니다. 모바일에는 가로 스크롤 단계 표시와 단일 열 입력을 제공하고, 단계 변경 시 제목에 키보드 포커스를 전달합니다.
