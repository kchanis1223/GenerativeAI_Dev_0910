...
## 앱 키 분리 확인 (2026-09-11)

새 TMAP 키로 Full Text Geocoding HTTP 200과 주소 1건 응답을 확인했다. 새 키의 TMS 앱에는 별도 데이터가 조회되어, 시연 데이터를 등록한 기존 키를 `TMS_APP_KEY`로 보존했다. 기존 TMS 앱에는 센터 1곳·권역 1개·차량 5대·주문 6건을 등록하고 조회했다. 주문 부피 6건이 0으로 반환되는 차이는 남아 있어 `TMS_MASTER_VERIFIED=0`을 유지한다. 배차는 아직 실행하지 않았다.
